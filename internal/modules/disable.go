...
func GetRunningResourcesOfCommunityModule(ctx context.Context, repo repo.ModuleTemplatesRepository, module string) ([]string, clierror.Error) {
	modulesToUninstall, err := repo.CommunityInstalledByName(ctx, module)
	if err != nil {
		return nil, clierror.Wrap(err, clierror.New("failed to retrieve a list of installed community modules"))
	}
	if len(modulesToUninstall) == 0 {
		return nil, clierror.New("failed to find installed module")
	}
	if len(modulesToUninstall) > 1 {
		return nil, clierror.New("failed to determine module - multiple versions installed")
	}

	moduleToUninstall := modulesToUninstall[0]
	runningResources, err := repo.RunningAssociatedResourcesOfModule(ctx, moduleToUninstall)
	if err != nil {
...
func disableCommunity(writer io.Writer, ctx context.Context, repo repo.ModuleTemplatesRepository, module string) clierror.Error {
	...
	if err != nil {
		return clierror.Wrap(err, clierror.New(fmt.Sprintf("failed to determine version for module %v", module)))
	}
	...
	// We want to remove the resources in the reversed order
	...
	for _, resource := range moduleResources {
		...
		if err != nil {
			...
			fmt.Fprintf(writer, "failed to delete resource %s (%s)\n", r.GetName(), r.GetKind())
			continue
		}
...
func getModuleTemplateToDelete(ctx context.Context, repo repo.ModuleTemplatesRepository, module string) (*kyma.ModuleTemplate, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to retrieve a list of installed community modules")
	}
